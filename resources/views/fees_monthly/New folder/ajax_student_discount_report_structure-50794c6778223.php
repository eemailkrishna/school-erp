<div class="col-md-12">
<div class="col-md-6">
<center><button type="button" class="btn default" onclick="exportTableToExcel('printTable', 'Discount Report Structurewise')"><i class="fa fa-print" aria-hidden="true"></i>Print In Excel</button></center>
</div>
<div class="col-md-6">
<center><button type="button" class="btn default" onclick="for_print();"><i class="fa fa-print" aria-hidden="true"></i>Print In Pdf</button></center>
</div>
</div>

        <div class="col-md-12">
          <!-- /.box -->
          <div class="box">
            <!-- /.box-header -->
			<div class="col-md-10 col-md-offset-1">
            <div class="box-body table-responsive" id="printTable">
						<table cellspacing="0" cellpadding="5px;" class="" style="width:100%">
			<tr>
			<td colspan="3"><span style="font-size:20px;font-weight:bold"><center><b>SIMPTION TECH PVT LTD</b></center></span></td>
			</tr>
			<tr>
			<td style="float:left"><b>Dise Code : 23260100164</b></td>
			<td><center><b>STUDENT DISCOUNT STRUCTUREWISE REPORT</b></center></td>
			<td style="float:right"><b>School Code : 50702</b></td>
			</tr>
			<tr>
			<td colspan="3"><center><b>Classwise</b></center></td>
			</tr>
			</table>
				  
				  				  
				  <table id="example1" class="table table-bordered table-striped" border="1px" cellpadding="10px" cellspacing="0" width="100%">
						<thead class="my_background_color">
								<tr>
								  <th rowspan="3">S.No.</th>
								  <th rowspan="3">Student Name</th>
								  <th rowspan="3">Father Name</th>
								  <th rowspan="3">Class (Sec)</th>
								  <th rowspan="3">Contact No</th>
								  <th rowspan="3">Months</th>
								  								  <th>annual</th>
								  								  <th>singing</th>
								  								  <th>Transport fees</th>
								  								  <th>sports fees</th>
								  								  <th>exam fee</th>
								  								  <th>event fee</th>
								  								  <th>bus fee</th>
								  								  <th rowspan="3">Total Discount Amount</th>
								</tr>
								<tr>
								  <th colspan="7">T = Type, M = Method, A = Amount</th>
								</tr>
								<tr>
								  								  <th>( T / M / A )</th>
								  								  <th>( T / M / A )</th>
								  								  <th>( T / M / A )</th>
								  								  <th>( T / M / A )</th>
								  								  <th>( T / M / A )</th>
								  								  <th>( T / M / A )</th>
								  								  <th>( T / M / A )</th>
								  								</tr>
						</thead>
					<tbody>
										<tr>
					<td rowspan="12">1.</td>
					<td rowspan="12">Aamna Hussain</td>
					<td rowspan="12">Syed Farhat Hussain</td>
					<td rowspan="12">NURSERY (A)</td>
					<td rowspan="12">9018501010</td>
					<td>1. April Month</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td>Staff Disc / % / 2500</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td><span style="font-weight:bold;">2500</span></td>
					</tr>
										<tr>
					<td>2. May Month</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>3. June Month</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>4. July Month</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>5. August Month</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>6. September Month</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>7. October Month</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>8. November Month</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>9. December Month</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>10. January Month</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>11. February Month</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>12. March Month</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td rowspan="12">2.</td>
					<td rowspan="12">aman soni</td>
					<td rowspan="12">ravi soni</td>
					<td rowspan="12">11TH (A)</td>
					<td rowspan="12">6387770743</td>
					<td>1. April Month</td>
										<td>Staff Disc / % / 100</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td><span style="font-weight:bold;">100</span></td>
					</tr>
										<tr>
					<td>2. May Month</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>3. June Month</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>4. July Month</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>5. August Month</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>6. September Month</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>7. October Month</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>8. November Month</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>9. December Month</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>10. January Month</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>11. February Month</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>12. March Month</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td> /  / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td rowspan="12">3.</td>
					<td rowspan="12">AUGUSTAY</td>
					<td rowspan="12"> SHARMA</td>
					<td rowspan="12">10TH (A)</td>
					<td rowspan="12">9897015271</td>
					<td>1. April Month</td>
										<td>None / % / 0</td>
										<td>Sibling Di / Rs / 50</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td><span style="font-weight:bold;">50</span></td>
					</tr>
										<tr>
					<td>2. May Month</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>3. June Month</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>4. July Month</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>5. August Month</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>6. September Month</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>7. October Month</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>8. November Month</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>9. December Month</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>10. January Month</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>11. February Month</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>12. March Month</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td rowspan="12">4.</td>
					<td rowspan="12">Rahul Kumar</td>
					<td rowspan="12">Lalbabu Ray</td>
					<td rowspan="12">NURSERY (A)</td>
					<td rowspan="12">9570503057</td>
					<td>1. April Month</td>
										<td>None / % / 0</td>
										<td>Sibling Di / % / 50</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td><span style="font-weight:bold;">50</span></td>
					</tr>
										<tr>
					<td>2. May Month</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>3. June Month</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>4. July Month</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>5. August Month</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>6. September Month</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>7. October Month</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>8. November Month</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>9. December Month</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>10. January Month</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>11. February Month</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										<tr>
					<td>12. March Month</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td>None / % / 0</td>
										<td><span style="font-weight:bold;">0</span></td>
					</tr>
										</tbody>
					<tfoot>
					<tr>
					<td colspan="6"><span style="float:right;font-weight:bold;">Total Amount = </span></td>
										<td><span style="font-weight:bold;">100</span></td>
										<td><span style="font-weight:bold;">100</span></td>
										<td><span style="font-weight:bold;">0</span></td>
										<td><span style="font-weight:bold;">2500</span></td>
										<td><span style="font-weight:bold;">0</span></td>
										<td><span style="font-weight:bold;">0</span></td>
										<td><span style="font-weight:bold;">0</span></td>
										<td><span style="font-weight:bold;">2700</span></td>
					</tr>
					</tfoot>
				 </table>
			  
        <!-- /.col -->
      </div>
      </div>
			  <div class="col-md-12">&nbsp;</div>
			  <div class="col-md-12">
			  <div class="col-md-6">
			  <center><button type="button" class="btn my_background_color" onclick="exportTableToExcel('printTable', 'Discount Report Structurewise')"><i class="fa fa-print" aria-hidden="true"></i>  Print In Excel</button></center>
			  </div>
			  <div class="col-md-6">
			  <center><button type="button" class="btn my_background_color" onclick="for_print();"><i class="fa fa-print" aria-hidden="true"></i>  Print In Pdf</button></center>
			  </div>
			  </div>
  </div>
    
  </div>