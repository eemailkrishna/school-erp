...
function for_msg() {
    ...
    $("#student_message").val("Dear Parents, Kindly deposit the " + month_arr +
        " transport due amount ? of your child ? at the earliest. Regards, " + school_name);
}
    ...
    if (student_roll_no != '') {
        ...
        if (student_class != '' && student_class_section != '' && selected_month != '') {
            $.ajax({
                type: "POST",
                url: access_link +
                    "fees_monthly/ajax_get_selected_student_transport_list_for_print_statuswise.php",
                cache: false,
                data: {
                    student_class: student_class,
                    student_class_section: student_class_section,
